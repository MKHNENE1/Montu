...
function countItems1($item , $table ,$WHERE,$VALUE) {
	...
	$stmt4 = $con->prepare("
	(SELECT COUNT($item) FROM $table Where $WHERE = $VALUE AND regst = 5 LIMIT 5 )
	");
	...
	function countItems2($item , $table ,$WHERE,$VALUE) {
		...
		$stmt2 = $con->prepare("
		(SELECT COUNT($item) FROM $table Where $WHERE = $VALUE AND regst = 3 LIMIT 3  )
		");
...
(SELECT * FROM player Where regst = 5 LIMIT 5 )
UNION
(SELECT * FROM player Where regst = 3 LIMIT 3 )
...
<head>
  <meta charset="UTF-8">
  <title>CodePen - Table concept</title>
...
<?php }}else{
	echo "<h1><center>there is no request from players</center></h1>" ;
}} ?>